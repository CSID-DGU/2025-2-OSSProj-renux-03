...
static public class EtcApis
{
    static public void AddEtcApis(this WebApplication application)
    {
        ...
        app.MapGet("/major", async (ServerDbContext db) =>
        {
            var majors = await db.Majors
                .Select(major => new MajorDto(major.Id, major.Majorname))
                .ToListAsync();
        ...
        app.MapGet("/orgs", async (ServerDbContext db) =>
        {
            ...
            List<OrganizationDto> orgDtos = organizations
